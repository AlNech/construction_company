...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('admin.applications.index') }}" class="px-4 py-2 bg-gray-300 hover:bg-gray-400 rounded-md text-gray-800 transition duration-200">
                Назад
            </a>
        </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1">
                            @if($application->is_processed)
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Обработано</span>
                            @else
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Новая</span>
                            @endif
                        </p>
                    </div>
                ...
                @if(!$application->is_processed)
                    <div class="mt-6 flex justify-end">
                        <form action="{{ route('applications.markAsProcessed', $application) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-md text-white transition duration-200 flex items-center">
