...
@section('header', 'Редактировать проект: ' . $portfolio->title)
...
<div class="bg-white shadow rounded-lg overflow-hidden">
    <form action="{{ route('admin.portfolio.update', $portfolio) }}" method="POST" enctype="multipart/form-data">
        ...
        @method('PUT')
        <div class="p-6">
            @include('admin.portfolio.form', ['portfolio' => $portfolio, 'services' => $services])
        </div>
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end">
            <a href="{{ route('admin.portfolio.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                Отмена
            </a>
