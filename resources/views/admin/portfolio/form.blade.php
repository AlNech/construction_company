...
<div class="space-y-6">
    ...
    <div>
        ...
        <select name="service_id" id="service_id" required
            ...
            @foreach($services as $service)
                <option
                    value="{{ $service->id }}" {{ old('service_id', $portfolio->service_id ?? '') == $service->id ? 'selected' : '' }}>
                    {{ $service->title }}
                </option>
            @endforeach
        </select>
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
        ...
        <div>
            ...
            <input type="date" name="project_date" id="project_date"
                   value="{{ old('project_date', isset($portfolio) ? $portfolio->project_date->format('Y-m-d') : '') }}"
                   required
                   class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            @error('project_date')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
    ...
    <div>
        ...
        <select name="is_active" id="is_active"
                class="mt-1 block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
            <option
                value="1" {{ old('is_active', isset($portfolio) ? $portfolio->is_active : true) ? 'selected' : '' }}>
                Активен
            </option>
            <option
                value="0" {{ !old('is_active', isset($portfolio) ? $portfolio->is_active : true) ? 'selected' : '' }}>
                Неактивен
            </option>
        </select>
    </div>
